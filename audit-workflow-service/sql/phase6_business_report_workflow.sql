...
SET workflow_name = '报告合规审核工作流',
    description = '阶段6工作流：整份待审阅报告结合知识库检索结果交给 Qwen 大模型审核，并返回业务系统要求的 findings JSON',
    knowledge_binding = JSON_OBJECT(
      ...
      'default_scope', JSON_OBJECT(
        'knowledge_base_codes', JSON_ARRAY('default'),
        'effective_only', true
      ),
    ...
    prompt_template = '## 角色与核心任务
你是专业报告审核员，严格依据**知识库搜索返回的审核依据内容**，对用户提供的待审阅报告进行全面合规审查。
## 严格约束规则
唯一判定标准：仅以本次给到的知识库搜索结果为全部审核依据，禁止调用自身知识库、行业经验、主观认知、外部规则进行额外判定；无对应依据的内容不判定为问题。
内容审查要求：逐段比对报告原文与审核依据，全面筛查不符、缺失、错误、违规、表述不一致等全部问题，不遗漏、不夸大、不虚构问题。
...
内容禁止项：不得新增整改建议、优化方案、原因分析、评价性话术、多余解释、开场白、结束语、礼貌用语。
## 输出规范
请将审核结果整理为严格 JSON。
禁止输出 Markdown。
禁止输出 ```json 代码块。
...
"findings": []
    ...
    output_schema = JSON_OBJECT(
      'type', 'object',
      'required', JSON_ARRAY('success', 'summary', 'totalIssues', 'findings'),
