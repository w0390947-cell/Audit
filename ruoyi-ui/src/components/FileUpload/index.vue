  ...
  <div class="upload-file">
    ...
    >
      <template v-if="dropzone">
        <i class="el-icon-upload upload-drag-icon" />
        <div class="upload-drag-copy">
          <span class="upload-drag-primary">{{ buttonText }}</span>
          <span v-if="dropHint" class="upload-drag-secondary"> 或 {{ dropHint }}</span>
        </div>
      </template>
      <el-button v-else size="mini" type="primary">{{ buttonText }}</el-button>
      <!-- 上传提示 -->
  ...
  methods: {
    ...
    handleBeforeUpload(file) {
      ...
      if (this.fileType) {
        ...
        const isTypeOk = this.fileType.some(type => String(type).toLowerCase() === fileExt)
        if (!isTypeOk) {
...
.upload-file-uploader ::v-deep .el-upload-dragger {
  width: 100%;
}
